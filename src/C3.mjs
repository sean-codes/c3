...
import { FBXLoader } from '../libs/three/examples/jsm/loaders/FBXLoader.mjs'
...
export class C3 {
   constructor(options = undefined) {
      ...
      this.gamepad = this.headless ? null : new C3_Gamepad(this)
      this.network = new C3_Network(this)
      this.keyboard = this.headless ? null : new C3_Keyboard(this)
      this.transform = this.headless ? null : new C3_Transform(this)
      this.html = this.headless ? null : new C3_Html(this)
      this.math = C3_Math
      this.Model = C3_Model
      ...
      this.mouse = this.headless ? null : new C3_Mouse(this) 
   ...
   init({
      ...
      if(!this.headless) this.keyboard.applyKeyMap(keyMap)
      this.running = true
      this.loops = 0
   ...
   loadModels(models) {
      ...
      return new Promise((yay, nay) => {   
         ...
         for (const loadInfo of models) {
            ...
            console.log('load modle', loadInfo)
            var buffer = readFileSync(loadInfo.file, { encoding: 'utf-8' })
            var data = loader.parse(buffer, loadInfo.file)
            console.log(data)
